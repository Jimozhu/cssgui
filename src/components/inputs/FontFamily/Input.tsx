...
interface Props extends EditorProps<FontFamilyType> {
  ...
  // new props add by EZ
  getFontData?: () => Promise<APIFontData>
  showVariable?: boolean
}

export function FontFamilyInput({ label, value, showVariable = false, getFontData, onChange, onRemove }: Props) {
  ...
  React.useEffect(() => {
    const getFontsData = async () => {
      const fn = getFontData ?? getTypeFaceFontsData;
      const fontData = await fn()
...
const getTypeFaceFontsData = async (): Promise<APIFontData> => {
